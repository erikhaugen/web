...
package com.livescribe.aws.evernote.oauth;
...
public class SimpleOAuthRequest {
	 ...
	 * 
	 * @param providerUrl
	 *            the base URL to request a Request or Access token
	 * @param consumerKey
	 *            the OAuth consumer key, given by the Service Provider
	 * @param consumerSecret
	 *            the OAuth consumer secret, given by the Provider
	 * @param tokenSecret
	 *            if non-null, this is the previous oauth_token_secret that
	 *            should be used in signing this request. If null, this will
	 *            assume that this message does not include a token secret in
	 *            its signature
	 */
	public SimpleOAuthRequest(String providerUrl, String consumerKey,
		...
		setParameter("oauth_timestamp", Long.toString(System.currentTimeMillis() / 1000));
	...
	public Map<String, String> sendRequest() throws IOException {
		...
		if (responseCode != HttpURLConnection.HTTP_OK) {
			throw new IOException("Server returned error code: " + responseCode + " " + connection.getResponseMessage());
		}
		...
		for (String param : result.split("&")) {
			...
			if (equalsAt > 0) {
				...
				String value = URLDecoder.decode(param.substring(equalsAt + 1), "UTF-8");
