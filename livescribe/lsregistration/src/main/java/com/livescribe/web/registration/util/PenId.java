...
public class PenId {
    ...
    /**
     * Factory method to create PenId object from serialNumber or displayId
     * 
     * @param penId - penId is either in format of AYE-XXX-XXX-XX or 2594160246880
     * @return
     * @throws InvalidParameterException
     */
    public static PenId getPenIdObject(String penId) throws InvalidParameterException {
    	
    	if (penId == null) {
    		throw new InvalidParameterException("penId parameter is null.");
    	}
    	
    	try {
	    	if (penId.matches(PEN_DISPLAY_ID_PATTERN)) {
	    		return new PenId(penId);
	    	}
	    	
	    	if (penId.matches(PEN_SERIAL_NUMBER_PATTERN)) {
	    		return new PenId(Long.parseLong(penId));
	    	}
    	
    	} catch (IllegalArgumentException e) {
			throw new InvalidParameterException("The provided pen ID is not valid. " + e.getMessage());
		}
    	
    	throw new InvalidParameterException("Invalid penId");
    }
...
//        System.out.println("toSerial: id % 877=" + (id % 877));
    ...
    public static void main(String[] args) {
    	System.out.println("AYE-AAA-ADG-AB".matches(PEN_DISPLAY_ID_PATTERN));
    	System.out.println("25941602468801".matches(PEN_SERIAL_NUMBER_PATTERN));
    }
