...
public class ${declarationName}Dao extends ${pojo.importType("com.livescribe.framework.orm.lspaperclouddb.AbstractDao")} {
...
<#else>    
    ...
    public void persist(${declarationName} transientInstance) {
        logger.debug("persisting ${declarationName} instance");
        try {
            ${sessionFactoryName}.getCurrentSession().persist(transientInstance);
            logger.debug("persist successful");
        }
        catch (RuntimeException re) {
            logger.error("persist failed", re);
    ...
    public void attachDirty(${declarationName} instance) {
        ...
        try {
            ${sessionFactoryName}.getCurrentSession().saveOrUpdate(instance);
    ...
    public void attachClean(${declarationName} instance) {
        ...
        try {
            ${sessionFactoryName}.getCurrentSession().lock(instance, ${pojo.importType("org.hibernate.LockMode")}.NONE);
            logger.debug("attach successful");
        }
        catch (RuntimeException re) {
            logger.error("attach failed", re);
    ...
    public void delete(${declarationName} persistentInstance) {
        ...
        try {
            ${sessionFactoryName}.getCurrentSession().delete(persistentInstance);
...
//        logger.debug("merging ${declarationName} instance");
        try {
            ${declarationName} result = (${declarationName}) ${sessionFactoryName}.getCurrentSession()
...
<#if clazz.identifierProperty?has_content>
    public ${declarationName} findById( ${c2j.getJavaTypeName(clazz.identifierProperty, jdk5)} id) {
        logger.debug("getting ${declarationName} instance with id: " + id);
        try {
            ${declarationName} instance = (${declarationName}) ${sessionFactoryName}.getCurrentSession()
...
<#else>
    public ${pojo.importType("java.util.List")} findByExample(${declarationName} instance) {
        ...
        try {
<#if jdk5>
            ${pojo.importType("java.util.List")}<${declarationName}> results = (List<${declarationName}>) ${sessionFactoryName}.getCurrentSession()
<#else>
            ${pojo.importType("java.util.List")} results = ${sessionFactoryName}.getCurrentSession()
</#if>
                    .createCriteria("${clazz.entityName}")
<#if jdk5>
...
<#else>
    public ${pojo.importType("java.util.List")} ${methname}(${argList}) {
        ...
        ${pojo.importType("org.hibernate.Query")} query = ${sessionFactoryName}.getCurrentSession()
